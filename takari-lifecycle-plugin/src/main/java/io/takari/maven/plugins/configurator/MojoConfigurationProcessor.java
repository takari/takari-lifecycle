...
// provided by the POM, but we need to change the procesing in order to support the namespacing of configuration in the
// POM
// configuraiton. So we take the original configuration, take the POM configuration and take out the configuration that
// is namespaced to
...
public class MojoConfigurationProcessor {
     ...
     * @throws ComponentConfigurationException
     */
    PlexusConfiguration extractAndMerge(
            String goal, PlexusConfiguration pluginConfigurationFromMaven, PlexusConfiguration defaultMojoConfiguration)
            throws ComponentConfigurationException {
        //
        ...
        return new XmlPlexusConfiguration(
                Xpp3Dom.mergeXpp3Dom(convert(mojoConfigurationFromPom), convert(defaultMojoConfiguration)));
    }

    PlexusConfiguration mergePlexusConfiguration(PlexusConfiguration dominant, PlexusConfiguration recessive)
            throws ComponentConfigurationException {
