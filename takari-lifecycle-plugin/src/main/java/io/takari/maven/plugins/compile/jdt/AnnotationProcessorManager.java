...
class AnnotationProcessorManager extends BaseAnnotationProcessorManager {
    ...
    public void reportProcessorException(Processor p, Exception e) {
        String msg = String.format(
                "Exception executing annotation processor %s: %s", p.getClass().getName(), e.getMessage());
        ...
        private TypeElementImpl getTopLevelType(ElementImpl element) {
            for (; element != null; element = (ElementImpl) element.getEnclosingElement()) {
                if (element instanceof TypeElementImpl
                        && ((TypeElementImpl) element).getNestingKind() == NestingKind.TOP_LEVEL) {
    ...
    public void processAnnotations(
            CompilationUnitDeclaration[] units, ReferenceBinding[] referenceBindings, boolean isLastRound) {
