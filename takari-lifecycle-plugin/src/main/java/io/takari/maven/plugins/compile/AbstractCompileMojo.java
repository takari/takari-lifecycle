...
public abstract class AbstractCompileMojo extends AbstractMojo {
                ...
                for (String include : includes) {
                    ...
                    if (!illegal.isEmpty()) {
                        throw new MojoExecutionException(String.format(
                                "<includes> patterns must end with %s. Illegal patterns: %s",
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        try {
            ...
            boolean classpathChanged =
                    compiler.setClasspath(classpath, getMainOutputDirectory(), getDirectDependencies());
    ...
    private List<File> getSourcepath(Proc proc) throws MojoExecutionException {
        ...
        if (privatePackageReference != AccessRulesViolation.ignore) {
            // dependency export-packages are calculated for classes folder, information is not available for source
            // folders
        ...
        if (!unsupportedDependencies.isEmpty()) {
            throw new MojoExecutionException(
                    "Unsupported <sourcepath> classified reactor dependencies: " + unsupportedDependencies);
        }
    ...
    private Proc getEffectiveProc(List<File> classpath, List<File> processorpath) {
        ...
        if (proc == null) {
                ...
                if (jar.isFile()) {
                    try (ZipFile zip = new ZipFile(jar)) {
                        ...
                        if (entry != null) {
                            try (BufferedReader r = new BufferedReader(
                                ...
                                if (!lines.isEmpty()) {
                                    processors
                                            .computeIfAbsent(jar, k -> new ArrayList<>())
                                            .addAll(lines);
                                }
                            }
                        }
                    ...
                    try {
                        List<String> lines = Files.readAllLines(
                                new File(jar, "META-INF/services/javax.annotation.processing.Processor").toPath(),
            ...
            if (!processors.isEmpty()) {
                StringBuilder msg =
                        new StringBuilder("<proc> must be one of 'none', 'only' or 'proc'. Processors found: ");
