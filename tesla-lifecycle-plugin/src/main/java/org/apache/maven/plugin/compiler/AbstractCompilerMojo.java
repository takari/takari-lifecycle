...
public abstract class AbstractCompilerMojo extends AbstractMojo {
  ...
  public void execute() throws MojoExecutionException, CompilationFailureException {
     ...
     * 
     * Set<File> staleSources; try { staleSources = computeStaleSources(compilerConfiguration,
     * compiler, getSourceInclusionScanner(staleMillis));
     * 
     * canUpdateTarget = compiler.canUpdateTarget(compilerConfiguration);
     * 
     * if
     * (compiler.getCompilerOutputStyle().equals(CompilerOutputStyle.ONE_OUTPUT_FILE_FOR_ALL_INPUT_FILES
     * ) && !canUpdateTarget) { getLog().info("RESCANNING!"); // TODO: This second scan for source
     * files is sub-optimal String inputFileEnding =
     * compiler.getInputFileEnding(compilerConfiguration);
     * 
     * sources = computeStaleSources(compilerConfiguration, compiler,
     * getSourceInclusionScanner(inputFileEnding));
     * 
     * compilerConfiguration.setSourceFiles(sources); } else {
     * compilerConfiguration.setSourceFiles(staleSources); } } catch (CompilerException e) { throw
     * new MojoExecutionException("Error while computing stale sources.", e); }
     * 
