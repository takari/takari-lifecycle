...
public abstract class AbstractCompilerMojo extends AbstractMojo {
  ...
  public void execute() throws MojoExecutionException, CompilationFailureException {
    ...
    if (debug && StringUtils.isNotEmpty(debuglevel)) {
      ...
      for (int i = 0; i < split.length; i++) {
        if (!(split[i].equalsIgnoreCase("none") || split[i].equalsIgnoreCase("lines") || split[i].equalsIgnoreCase("vars") || split[i].equalsIgnoreCase("source"))) {
  ...
  protected CompilerResult convertToCompilerResult(List<CompilerError> compilerErrors) {
    ...
    for (CompilerError compilerError : compilerErrors) {
      messages.add(new CompilerMessage(compilerError.getFile(), compilerError.getKind(), compilerError.getStartLine(), compilerError.getStartColumn(), compilerError.getEndLine(), compilerError
          .getEndColumn(), compilerError.getMessage()));
  ...
  private String getMemoryValue(String setting) {
    ...
    if (isDigits(setting)) {
      value = setting + "m";
    } else if ((isDigits(setting.substring(0, setting.length() - 1))) && (setting.toLowerCase().endsWith("m"))) {
      value = setting;
    }
    return value;
  }
